...
public class MemberServlet extends MyUploadServlet {
	...
	protected void process(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (uri.indexOf("login.do") != -1) {
			...
			memberList(req, resp);
		} else if (uri.indexOf("resign.do") != -1) {
			resign(req, resp);
		}
	}
	...
	private void loginSubmit(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (dto != null) {
			if (pwd.equals(dto.getPwd()) && !dto.getType().equals("d")) {
	...
	private void memberList(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		MemberDAO dao = new MemberDAO();
		...
		// 데이터 개수
		int dataCount = dao.dataCount(type);
		...
		// 회원목록 가져오기
		List<MemberDTO> list = dao.listMember(offset, rows, type);
		...
		String listUrl = cp + "/member/memberList.do";
		...
		forward(req, resp, "/WEB-INF/views/member/memberList.jsp");
	}

	private void resign(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		MemberDAO dao = new MemberDAO();
		HttpSession session = req.getSession();
		String result;
		...
		try {
			...
			if (dao.deleteMember(dto, req.getParameter("currentPwd")) == 0) {
				result = "현재 비밀번호를 확인해주세요.";
				req.setAttribute("result", result);
				forward(req, resp, "/WEB-INF/views/member/resign.jsp");
				return;
			}
